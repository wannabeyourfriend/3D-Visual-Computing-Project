...
def emd_approx(sample, ref):
    ...
    Args:
        sample: 形状为[B, N, 3]的点云张量
        ref: 形状为[B, M, 3]的点云张量
        
    Returns:
        形状为[B]的EMD距离张量
    """
    global _EMD_NOT_IMPL_WARNED, _EMD_IMPORT_FAILED
    
    if _EMD_AVAILABLE:
        try:
            # 确保点云有相同数量的点
            if sample.size(1) != ref.size(1):
                # 如果点数不同，随机采样到相同数量
                if sample.size(1) > ref.size(1):
                    # 从sample中随机采样到ref的点数
                    idx = torch.randperm(sample.size(1))[:ref.size(1)].to(sample.device)
                    sample = sample[:, idx, :]
                else:
                    # 从ref中随机采样到sample的点数
                    idx = torch.randperm(ref.size(1))[:sample.size(1)].to(ref.device)
                    ref = ref[:, idx, :]
            
            # 计算EMD距离，返回的是总距离，需要除以点数得到平均距离
            emd = earth_mover_distance(sample, ref, transpose=False)
            return emd / sample.size(1)  # 归一化为平均距离
        except Exception as e:
            if not _EMD_NOT_IMPL_WARNED:
                _EMD_NOT_IMPL_WARNED = True
                print('\n\n[WARNING]')
                print(f'  * EMD计算出错: {str(e)}')
                print('  * 将使用零值替代EMD')
                print('\n')
            return torch.zeros([sample.size(0)]).to(sample)
    else:
        if not _EMD_NOT_IMPL_WARNED:
            _EMD_NOT_IMPL_WARNED = True
            print('\n\n[WARNING]')
            if _EMD_IMPORT_FAILED:
                print('  * 无法导入EMD模块，请安装PyTorchEMD:')
                print('  * git clone https://github.com/daerduoCarey/PyTorchEMD')
                print('  * cd PyTorchEMD && python setup.py install')
            else:
                print('  * EMD未实现，可能是由于GPU兼容性问题')
            print('  * 将使用零值替代EMD')
            print('\n')
        return torch.zeros([sample.size(0)]).to(sample)
